...
class TeamsViewTests(TransactionTestCase):
    def setUp(self):
        self.c = Client()
        user = models.User.objects.create_user(username='admin', password='toortoor', is_superuser=True)
    ...
    def test_team_correct(self):
        ...
        self.c.post(self.path, 
            {'type':'team', 'name':'Team1', 'subnet':'192.168.1.0',
             'netmask':'255.255.255.0'})
        self.assertEqual(models.Team.objects.count(), 1)
        self.c.post(self.path,
            {'type':'team', 'name':'Team2', 'subnet':'192.168.2.0', 
             'netmask':'255.255.255.0'})
        self.assertEqual(models.Team.objects.count(), 2)
        # Smaller subnets
        self.c.post(self.path,
            {'type':'team', 'name':'Team3', 'subnet':'192.168.3.0',
             'netmask':'255.255.255.128'})
        self.assertEqual(models.Team.objects.count(), 3)
        self.c.post(self.path,
            {'type':'team', 'name':'Team4', 'subnet':'192.168.3.129',
             'netmask':'255.255.255.128'})
    ...
    def test_team_edit(self):
        ...
        post_data =  {'id':t.pk, 'type':'team', 'name':'Team2',
            'subnet':'192.168.1.0', 'netmask':'255.255.255.0'}
        ...
        post_data['subnet'] = '192.168.2.0'
        self.c.post(self.path, post_data)
        t = models.Team.objects.get(pk=t.pk) # Reload from DB
        self.assertEqual(t.subnet, '192.168.2.0')

        post_data['netmask'] = '255.255.255.128'
        self.c.post(self.path, post_data)
        t = models.Team.objects.get(pk=t.pk) # Reload from DB
